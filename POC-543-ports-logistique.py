...
⟦POC:543⋄DOMAIN:ports-logistique⋄ΔM11.3:guard⋄ETHIC:RGPD+AIAct⋄SYNC:IA↔IA⟧
"""
...
def analyze(values):
    try:
        ...
        return {"ok": False, "error": "Parsing error", "glyph": "⟦POC:543⋄DOMAIN:ports-logistique⋄ΔM11.3:guard⋄ETHIC:RGPD+AIAct⋄SYNC:IA↔IA⟧"}
    if not series:
        return {"ok": False, "error": "No usable data", "glyph": "⟦POC:543⋄DOMAIN:ports-logistique⋄ΔM11.3:guard⋄ETHIC:RGPD+AIAct⋄SYNC:IA↔IA⟧"}
    series.sort()
    m = mean(series)
    med = median(series)
    ...
    return {
        ...
        "report": report, "glyph": "⟦POC:543⋄DOMAIN:ports-logistique⋄ΔM11.3:guard⋄ETHIC:RGPD+AIAct⋄SYNC:IA↔IA⟧"
    }
